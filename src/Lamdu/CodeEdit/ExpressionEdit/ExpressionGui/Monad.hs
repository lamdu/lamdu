...
withParamName :: MonadA m => IsDependent -> Guid -> (Sugar.Name -> ExprGuiM m a) -> ExprGuiM m a
  ...
  if null storedName
    then do
      ...
      (Lens.over exprGuiM . RWS.local) (Lens.set aNameGen newNameGen) $
        useName (Sugar.Name Sugar.AutoGeneratedName name)
    else useName (Sugar.Name Sugar.StoredName storedName)

getDefName :: MonadA m => Guid -> ExprGuiM m Sugar.Name
getDefName = transaction . getGuidName

getGuidName :: MonadA m => Guid -> T m Sugar.Name
getGuidName guid = do
  storedName <- Transaction.getP $ Anchors.assocNameRef guid
  return $
    if null storedName
    then Sugar.Name Sugar.AutoGeneratedName $ (("anon_"++) . take 6 . Guid.asHex) guid
    else Sugar.Name Sugar.StoredName storedName

withNameFrom ::
  MonadA m =>
  Sugar.VarType -> Guid -> (Sugar.Name -> ExprGuiM m a) -> ExprGuiM m a
...
withNameFromParamGuid ::
  MonadA m => Guid -> (Sugar.Name -> ExprGuiM m a) -> ExprGuiM m a
withNameFromParamGuid = withNameFrom Sugar.GetParameter

withNameFromGetVar ::
  MonadA m => Sugar.GetVar Sugar.Name f ->
  (Sugar.Name -> ExprGuiM m a) -> ExprGuiM m a
