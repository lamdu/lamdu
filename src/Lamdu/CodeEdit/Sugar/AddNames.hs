...
pass0Result guid mName = do
  ...
  where
    myNameGuidMap =
      maybe mempty (`nameGuidMap_singleton` guid) mName
...
makeStoredNameEnv storedName storedNamesBelow guid env =
  ...
  where
    ...
    collidingGuids =
      ...
      nameGuidMap_lookup storedName storedNamesBelow
...
p1cpsNameConvertorLocal isDep guid storedNames =
  ...
  (`runState` p1env) . Lens.zoom p1NameGen $
    let
      noConflict name =
        name `nameGuidMap_notMember` storedNameGuidMapBelow &&
        name `Set.notMember` (p1env ^. p1StoredNames)
    in
      Name AutoGeneratedName NoCollision <$>
      NameGen.newName noConflict
      isDep guid
