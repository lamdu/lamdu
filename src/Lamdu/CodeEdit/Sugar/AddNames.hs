...
instance MonadNaming Pass0M where
  ...
  opMakeTagName = p0nameConvertor
  opDefName = p0nameConvertor
...
p0nameConvertor _ = pass0Result mempty

p0cpsNameConvertor :: CPSNameConvertor Pass0M
p0cpsNameConvertor _ mName = CPS $ \k -> do
...
p1GetNameGen :: Pass1M (NameGen Guid)
p1GetNameGen = Pass1M Reader.ask
p1WithNameGen :: NameGen Guid -> Pass1M a -> Pass1M a
p1WithNameGen newNameGen (Pass1M act) =
...
instance MonadNaming Pass1M where
  ...
  opRun = runPass1M <$> p1GetNameGen
  opWithDefName = p1cpsNameConvertorPrefix "def_"
  opWithParamName = p1cpsNameConvertor
  opWithWhereItemName = p1cpsNameConvertor NameGen.Independent
  opGetParamName _ (StoredNames (Just str) _) = pure $ Name StoredName str
  opGetParamName guid (StoredNames _ _) = do
    nameGen <- p1GetNameGen
  ...
  opMakeTagName = p1nameConvertor "tag_"
  opDefName = p1nameConvertor "def_"

makeNameByGuid :: String -> Guid -> StoredNames -> NameGen Guid -> (Name, NameGen Guid)
...
p1cpsNameConvertorHelper :: (NameGen Guid -> (a, NameGen Guid)) -> CPS Pass1M a
p1cpsNameConvertorHelper nameMaker = CPS $ \k -> do
  curNameGen <- p1GetNameGen
  let
    (name, newNameGen) = nameMaker curNameGen
  res <- p1WithNameGen newNameGen k
  return (name, res)

p1cpsNameConvertorPrefix :: String -> CPSNameConvertor Pass1M
p1cpsNameConvertorPrefix prefix guid storedNames =
  p1cpsNameConvertorHelper $ makeNameByGuid prefix guid storedNames

p1cpsNameConvertor :: NameGen.IsDependent -> CPSNameConvertor Pass1M
p1cpsNameConvertor isDep guid storedNames = p1cpsNameConvertorHelper $ makeName isDep guid storedNames

p1nameConvertor :: String -> NameConvertor Pass1M
p1nameConvertor _ _ (StoredNames (Just str) _) = pure $ Name StoredName str
