...
module Lamdu.Name
    ...
    , TagName(..), tnDisplayText, tnTagCollision, tnIsAutoGen
...
visible (NameTag (TagName disp tagCollision _)) = pure (disp, tagCollision)
...
-- | A valid text for actual names
isValidName :: Text -> Bool
isValidName name = isValidText name && not ("." `Text.isInfixOf` name)
