...
orderRecord :: Monad m => Order m (Sugar.Composite name im am a)
orderRecord = Sugar.cItems %%~ orderByTag (^. Sugar.ciTag . Sugar.tagInfo)

orderLabeledApply :: Monad m => Order m (Sugar.LabeledApply name im am a)
orderLabeledApply = Sugar.aAnnotatedArgs %%~ orderByTag (^. Sugar.aaTag)

orderCase :: Monad m => Order m (Sugar.Case name (T m) am a)
orderCase = Sugar.cBody %%~ orderRecord

orderLam :: Monad m => Order m (Sugar.Lambda name (T m) am a)
orderLam = Sugar.lamBinder orderBinder

orderBody :: Monad m => Order m (Sugar.Body name (T m) am a)
...
orderExpr :: Monad m => Order m (Sugar.Expression name (T m) am a)
orderExpr e =
    e
...
orderBinder :: Monad m => Order m (Sugar.Binder name (T m) am a)
orderBinder =
...
orderDef ::
    Monad m => Order m (Sugar.Definition name (T m) am (Sugar.Expression name (T m) am a))
orderDef def =
    def
