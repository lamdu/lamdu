...
instance Monad tm => MonadNaming (Pass2MakeNames tm) where
    ...
    opGetName Walk.ParamName (P1Out (MStoredName mName uuid) storedNamesUnder) =
        case mName of
            Just name ->
...
p2cpsNameConvertor (P1Out mStoredName storedNamesBelow) nameMaker =
    CPS $ \k ->
    do
        oldEnv <- p2GetEnv
        let (newName, newEnv) =
...
p2cpsNameConvertorGlobal prefix storedNames =
    ...
    \p2env -> (makeUUIDName prefix (storedNames ^. p1StoredName), p2env)
...
p2cpsNameConvertorLocal isFunction storedNames =
    ...
    where
        P1Out (MStoredName _ uuid) storedNamesBelow = storedNames

p2nameConvertor :: Monad tm => Text -> Walk.NameConvertor (Pass2MakeNames tm)
p2nameConvertor prefix (P1Out mStoredName storedNamesBelow) =
    case mName of
