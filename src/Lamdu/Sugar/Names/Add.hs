...
p2cpsNameConvertor (P1Out mName uuid storedNamesBelow) nameMaker =
    CPS $ \k ->
    do
        oldEnv <- p2GetEnv
        let (newName, newEnv) =
...
p2cpsNameConvertorLocal isFunction storedNames =
    ...
    where
        P1Out _ uuid storedNamesBelow = storedNames

p2nameConvertor :: Monad tm => Text -> Walk.NameConvertor (Pass2MakeNames tm)
p2nameConvertor prefix (P1Out mName uuid storedNamesBelow) =
    case mName of
