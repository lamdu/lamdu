...
loadRepl cache monitors annMode evalRes cp =
    do
        ...
        let replEntityId = expr ^. bContent . SugarLens.binderContentResultExpr . _PNode . ann . plEntityId
        pure Repl
            { _replExpr = expr
...
loadWorkArea cache monitors annMode evalRes cp =
    do
        ...
        panes <-
            ...
            (repl ^. replExpr . bContent . SugarLens.binderContentResultExpr . _PNode . ann . plEntityId)
        pure WorkArea
            { _waRepl = repl
            , _waPanes = panes
            , _waGlobals =
