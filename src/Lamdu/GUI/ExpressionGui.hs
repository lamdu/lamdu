...
nextHolesBefore ::
    ...
    NearestHoles
nextHolesBefore val =
    node ^. Sugar._Expr . Sugar.ann . Sugar.plData . plNearestHoles
    & if Lens.has (Sugar._Expr . Sugar.val . SugarLens.bodyUnfinished) node
        then NearestHoles.next ?~ node ^. Sugar._Expr . Sugar.ann . Sugar.plEntityId
        else id
    where
