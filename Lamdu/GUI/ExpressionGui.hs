...
makeNameEdit ::
  MonadA m => Sugar.NameProperty Name m -> Widget.Id -> ExprGuiM m (WidgetT m)
makeNameEdit (Sugar.NameProperty (Name nameSrc nameCollision name) setName) myId = do
  collisionSuffixes <-
...
makeNameView :: MonadA m => Name -> AnimId -> WE.WidgetEnvT m (Widget f)
makeNameView (Name nameSrc collision name) animId = do
