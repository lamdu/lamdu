...
instance MonadA tm => MonadNaming (Pass2M tm) where
    ...
    opGetName ParamName (StoredNames (MStoredName mName guid) storedNamesUnder) =
        case mName of
            Just name ->
            ...
            Nothing ->
                do
                    ...
                    let name src str = Name src NoCollision (setName guid) str
                    evalState (NameGen.existingName guid) nameGen
                        & maybe (name NameSourceOutOfScope "Error:OutOfScope!")
                          (name NameSourceAutoGenerated)
                        & pure
    opGetName TagName x = p2nameConvertor "tag_" x
    opGetName NominalName x = p2nameConvertor "nom_" x
    opGetName DefName x = p2nameConvertor "def_" x
...
fixVarToTags :: MonadA m => VarToTags -> T m ()
