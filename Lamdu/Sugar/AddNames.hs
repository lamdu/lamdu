...
type CPSNameConvertor m tm = Guid -> NameProperty (OldName m) tm -> CPS m (NameProperty (NewName m) tm)
type NameConvertor m tm = Guid -> NameProperty (OldName m) tm -> m (NameProperty (NewName m) tm)
...
pass0Result ::
  NameScope -> Guid -> NameProperty MStoredName tm ->
...
p1cpsNameConvertor ::
  Guid -> NameProperty StoredNames tm ->
