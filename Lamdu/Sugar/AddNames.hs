...
instance MonadNaming Pass1M where
  ...
  opGetParamName nameProp =
    nameProp & npName %%~
    \(StoredNames (MStoredName name) storedNamesUnder) ->
    case name of
      Just str ->
  ...
  opGetHiddenParamsName nameProp =
    nameProp & npName %%~
    \(StoredNames (MStoredName mName) _) ->
...
p1nameConvertor prefix (NameProperty storedNames guid setName) =
  mkNameProperty <$>
  case mName of
