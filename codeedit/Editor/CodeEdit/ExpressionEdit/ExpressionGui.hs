...
makeNameEdit (nameSrc, name) ident myId =
  ...
  VA.otransaction . flip makeEditor myId =<<
  VA.transaction (Anchors.assocNameRef ident)
  where
    makeEditor =
...
nameSrcTint :: VA.NameSource -> Widget f -> Widget f
nameSrcTint VA.AutoGeneratedName = Widget.tint Config.autoGeneratedNameTint
nameSrcTint VA.StoredName = id
