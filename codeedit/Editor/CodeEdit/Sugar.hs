...
convertApply :: Monad m => Data.Apply (SugarInfer.Result m) -> Convertor m
...
wrapOperatorHole ::
  Monad m => SugarInfer.Result m -> Expression m -> SugarM m (Expression m)
...
convertExpressionI :: Monad m => SugarInfer.Result m -> SugarM m (Expression m)
